...
// 1 line is added by Hao Fu.
using caffe::GpuStreamPool;
using caffe::Blob;
using caffe::Caffe;
using caffe::Net;
using caffe::Layer;
...
int test() {
  ...
  if (gpus.size() != 0) {
    ...
    Caffe::SetDevice(gpus[0]);
    Caffe::set_mode(Caffe::GPU);
    // 2 lines are added by Hao Fu.
    // Set the device that the GpuStreamPool object is resident on.
    GpuStreamPool::Get().SetDevice(gpus[0]);
    LOG(INFO) << "The GpuStreamPool object is resident on device " << gpus[0];
  } else {
    LOG(INFO) << "Use CPU.";
    Caffe::set_mode(Caffe::CPU);
  }
  ...
  // Reset the GpuStreamPool.
  if (gpus.size() != 0) {
    GpuStreamPool::Get().Reset();
    LOG(INFO) << "Reset the GpuStreamPool.";
  }
