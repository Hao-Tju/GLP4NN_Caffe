...
#ifndef METRIC_FUNC_HPP_
#define METRIC_FUNC_HPP_
...
#define CHECK_DRIVER_ERROR(apiFuncCall)               \
  do {                                              \
    CUresult _status = apiFuncCall;                 \
    if (_status != CUDA_SUCCESS) {                  \
      std::cerr << __FILE__ << ":" << __LINE__ << ": error: function " << #apiFuncCall << \
      " failed with error " << _status << ".\n"     \
    }                                               \
  } while (0)
...
namespace caffe {
  ...
  void InitMetric(const string metricName) {
    ...
    CHECK_CUPTI_ERROR(cuptiSubscribe(&subscriber, (CUpti_CallbackFunc)getMetricValueCallback, &metricData));
    ...
    CHECK_CUPTI_ERROR(cuptiMetricGetIdFromName(device, metricName.c_str(), &metricId));
    CHECK_CUPTI_ERROR(cuptiMetricGetNumEvents(metricId, &metricData.numEvents));
    ...
    CHECK_CUPTI_ERROR(cuptiMetricCreateEventGroupSets(context, sizeof(metricId), &metricId, &passData));
  ...
  void EndMetric() {
    ...
    size_t valueKindValue = sizeof(valueKind);
    ...
    CHECK_CUPTI_ERROR(cuptiUnsubscribe(subscriber));
  }
  ...
  void CUPTIAPI getMetricValueCallback(void *userdata, CUpti_CallbackDomain domain,
    ...
    if (cbInfo->callbackSite == CUPTI_API_ENTER) {
      ...
      CHECK_CUPTI_ERROR(cuptiSetEventCollectionMode(cbInfo->context, CUPTI_EVENT_COLLECTION_MODE_KERNEL));
      ...
      for (i = 0; i < metricData->eventGroups->numEventGroups; ++ i) {
        ...
        CHECK_CUPTI_ERROR(cuptiEventGroupEnable(metricData->eventGroups->eventGroups[i]));
      }
    }
    ...
    if (cbInfo->callbackSite == CUPTI_API_EXIT) {
      ...
      for (i = 0; i < metricData->eventGroups->numEventGroups; ++ i) {
        ...
        valueSize = sizeof(uint64_t) * numInstances;
      ...
      for (i = 0; i < metricData->eventGroups->numEventGroups; ++ i) {
        CHECK_CUPTI_ERROR(cuptiEventGroupDisable(metricData->eventGroups->eventGroups[i]));
      }
    }
  }
}
