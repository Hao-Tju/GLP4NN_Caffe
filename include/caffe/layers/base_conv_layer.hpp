...
#ifndef CPU_ONLY
  inline void conv_im2col_gpu(const Dtype* data, Dtype* col_buff, int stream_id = -1) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      ...
      im2col_nd_gpu(data, num_spatial_axes_, num_kernels_im2col_,
          conv_input_shape_.gpu_data(), col_buffer_[(stream_id == -1) ? 0 : stream_id]->gpu_shape(),
          kernel_shape_.gpu_data(), pad_.gpu_data(),
          stride_.gpu_data(), dilation_.gpu_data(), col_buff,
          stream_id);
    }
  }
  inline void conv_col2im_gpu(const Dtype* col_buff, Dtype* data, int stream_id = -1) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
