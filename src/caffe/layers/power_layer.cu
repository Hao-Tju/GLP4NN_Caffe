...
void PowerLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (scale_ != Dtype(1)) {
    caffe_gpu_scal(count, scale_, top_data, -1);
  }
  if (shift_ != Dtype(0)) {
  ...
  if (propagate_down[0]) {
    ...
    if (diff_scale_ == Dtype(0) || power_ == Dtype(1)) {
      ...
      if (power_ == Dtype(2)) {
        ...
        caffe_gpu_scal(count, power_, bottom_diff, -1);
      } else {
        caffe_copy(count, bottom_data, bottom_diff);
        if (scale_ != Dtype(1)) {
          caffe_gpu_scal(count, scale_, bottom_diff, -1);
        }
        if (shift_ != Dtype(0)) {
        ...
        if (diff_scale_ != Dtype(1)) {
          caffe_gpu_scal(count, diff_scale_, bottom_diff, -1);
        }
      }
    }
