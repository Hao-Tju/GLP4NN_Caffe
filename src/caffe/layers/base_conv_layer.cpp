...
void BaseConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  // Log<Dtype>::ConvParamInfoToLog(this->layer_param_, this->num_, this->conv_input_shape_,
  //    this->num_output_, this->kernel_shape_, this->stride_, this->pad_);
}
...
void BaseConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
  ...
  if (!is_1x1_) {
    if (!skip_im2col) {
      ...
      conv_im2col_cpu(input, col_buffer_[0]->mutable_cpu_data());
    }
    // Modified by Hao Fu.
    // col_buff = col_buffer_.cpu_data();
    col_buff = col_buffer_[0]->cpu_data();
  }
...
void BaseConvolutionLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
  ...
  Dtype* col_buff = col_buffer_[0]->mutable_cpu_data();
  if (is_1x1_) {
    col_buff = input;
  }
  for (int g = 0; g < group_; ++g) {
...
void BaseConvolutionLayer<Dtype>::SetColBufferNum (int buffer_num) {
  ...
  for (int i = col_buffer_.size(); i < buffer_num; ++ i) {
    if (i == (buffer_num - 1)) {
      ...
      InfoLog::Get().RecordInfoLog(this->layer_param_.name(), "ColBuffer-MEM", temp_ss.str());
...
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
  ...
  if (!is_1x1_) {
    if (!skip_im2col) {
      ...
      conv_im2col_gpu(input, col_buffer_[(stream_id == -1) ? 0 : stream_id]->mutable_gpu_data(stream_id), stream_id);
    }
...
void BaseConvolutionLayer<Dtype>::forward_gpu_bias(Dtype* output,
    const Dtype* bias, const Dtype* bias_multiplier, int stream_id) {
  //const Dtype* temp_bias_multiplier = bias_multiplier_[(stream_id == -1) ? 0 : stream_id]->gpu_data(stream_id);
  caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_,
      out_spatial_dim_, 1, (Dtype)1., bias, bias_multiplier,
      (Dtype)1., output, stream_id);
  //caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_,
...
void BaseConvolutionLayer<Dtype>::weight_gpu_gemm(const Dtype* input,
    const Dtype* output, Dtype* weights, int stream_id) {
  const Dtype* col_buff = input;
  ...
  for (int g = 0; g < group_; ++ g) {
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
        kernel_dim_, conv_out_spatial_dim_,
        (Dtype)1., output + output_offset_ * g, col_buff + col_offset_ * g,
        (Dtype)1., weights + weight_offset_ * g, stream_id);
  }
}

template <typename Dtype>
void BaseConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    const Dtype* input, int stream_id) {
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_output_, out_spatial_dim_, 1.,
      input, bias_multiplier_.gpu_data(), 1., bias, stream_id);
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
  ...
  if (!is_1x1_) {
    if (!skip_im2col) {
      conv_im2col_gpu(input, col_buffer_[0]->mutable_gpu_data());
    }
...
void BaseConvolutionLayer<Dtype>::backward_gpu_gemm(const Dtype* output,
  ...
  Dtype* col_buff = col_buffer_[0]->mutable_gpu_data();
  if (is_1x1_) {
    col_buff = input;
  }
  for (int g = 0; g < group_; ++g) {
