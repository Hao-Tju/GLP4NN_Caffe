...
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
  ...
  for (int stream_idx = 1; stream_idx <= parallel_degree; ++ stream_idx) {
    ...
    if (!is_1x1_) {
      if (!skip_im2col) {
        // Modified by Hao Fu.
        conv_im2col_gpu(input + (stream_idx - 1) * this->bottom_dim_, col_buffer_[stream_idx - 1]->mutable_gpu_data(), stream_idx);
      }
      // Modified by Hao Fu.
      col_buff = col_buffer_[stream_idx - 1]->gpu_data();
    }
    for (int g = 0; g < group_; ++g) {
      ...
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, conv_out_channels_ /
          group_, conv_out_spatial_dim_, kernel_dim_,
          (Dtype)1., weights + weight_offset_ * g, col_buff + col_offset_ * g,
          (Dtype)0., output + (stream_idx - 1) * this->top_dim_ + output_offset_ * g, stream_idx);
    }
  }
}
...
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm(const Dtype* input,
  ...
  if (!is_1x1_) {
    ...
    col_buff = col_buffer_[(stream_id == -1) ? 0 : stream_id]->gpu_data();
  }
  for (int g = 0; g < group_; ++g) {
    // TODO: Needed to consider the group_ argument.
    // Modified by Hao Fu.
...
void BaseConvolutionLayer<Dtype>::backward_gpu_gemm(const Dtype* output,
  ...
  for (int g = 0; g < group_; ++g) {
    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kernel_dim_,
        conv_out_spatial_dim_, conv_out_channels_ / group_,
        (Dtype)1., weights + weight_offset_ * g, output + output_offset_ * g,
        (Dtype)0., col_buff + col_offset_ * g, -1);
  }
  if (!is_1x1_) {
...
void BaseConvolutionLayer<Dtype>::weight_gpu_gemm(const Dtype* input,
  ...
  for (int g = 0; g < group_; ++g) {
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
        kernel_dim_, conv_out_spatial_dim_,
        (Dtype)1., output + output_offset_ * g, col_buff + col_offset_ * g,
        (Dtype)1., weights + weight_offset_ * g, -1);
  }
}
