  ...
  if ((!scale_param && propagate_down[1]) ||
    ...
    if (!is_eltwise) {
      ...
      if (inner_dim_ == 1) {
        ...
        if (scale_param) {
          Dtype result;
          caffe_gpu_dot(inner_dim_, product, sum_mult, &result, -1);
          *scale_diff += result;
        } else {
          caffe_gpu_dot(inner_dim_, product, sum_mult, scale_diff, -1);
        }
      } else {
        const Dtype* sum_mult = sum_multiplier_.gpu_data();
      ...
      if (outer_dim_ != 1) {
        ...
        if (scale_dim_ == 1) {
          ...
          if (scale_param) {
            Dtype result;
            caffe_gpu_dot(outer_dim_, sum_mult, sum_result, &result, -1);
            *scale_diff += result;
          } else {
            caffe_gpu_dot(outer_dim_, sum_mult, sum_result, scale_diff, -1);
          }
        } else {
