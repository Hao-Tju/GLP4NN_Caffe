...
void ReductionLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < num_; ++i) {
    ...
    case ReductionParameter_ReductionOp_MEAN:
      caffe_gpu_dot(dim_, mult_data, bottom_data, top_data, -1);
      break;
    case ReductionParameter_ReductionOp_ASUM:
      caffe_gpu_asum(dim_, bottom_data, top_data, -1);
      break;
    case ReductionParameter_ReductionOp_SUMSQ:
      caffe_gpu_dot(dim_, bottom_data, bottom_data, top_data, -1);
      break;
    default:
  ...
  if (coeff_ != Dtype(1)) {
    ...
    caffe_gpu_scal(num_, coeff_, top_data, -1);
  }
}
...
void ReductionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < num_; ++i) {
    ...
    case ReductionParameter_ReductionOp_ASUM:
      ...
      caffe_gpu_scal(dim_, bottom_coeff, bottom_diff, -1);
      break;
    case ReductionParameter_ReductionOp_SUMSQ:
      caffe_gpu_scale(dim_, 2 * bottom_coeff, bottom_data, bottom_diff, -1);
      break;
    default:
      LOG(FATAL) << "Unknown reduction op: "
          << ReductionParameter_ReductionOp_Name(op_);
