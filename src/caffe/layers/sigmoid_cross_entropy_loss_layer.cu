...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  if (normalization_ == LossParameter_NormalizationMode_VALID &&
      has_ignore_label_) {
    caffe_gpu_asum(count, count_data, &valid_count, -1);
  } else {
    valid_count = count;
  }
  Dtype loss;
  caffe_gpu_asum(count, loss_data, &loss, -1);
...
void SigmoidCrossEntropyLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    caffe_gpu_axpy(count, Dtype(-1), target, bottom_diff, -1);
    ...
    caffe_gpu_scal(count, loss_weight, bottom_diff, -1);
  }
}
